...
private extension ModelContainer {
    ...
    private class ModelContainerLoader {
        ...
        private typealias GamesDict = [Game.Builder: AttributeBuilderSet]
        private typealias AttributeDict = [Persistent.Model.Attribute.Builder: Persistent.Model.Attribute]
        ...
        public static func load(attributesSize: Int, _ container: ModelContainer) -> ModelContainer {
            ModelContainerLoader(container).populate(Attribute.Builder.random(attributesSize))
        }
        
        public static func load(gamesSize: Int, attributesSize: Int, _ container: ModelContainer) -> ModelContainer {
            let attributes: AttributeBuilderSet = Attribute.Builder.random(attributesSize)
            let games: GamesDict = .init(uniqueKeysWithValues: gamesSize.range.map { _ in
                (Game.Builder.random, attributes.subset(.random(in: 2...6)).asSet)
            })
        ...
        @discardableResult
        private func populate(_ attributes: AttributeBuilderSet, _ action: ((Key) -> ())? = nil) -> ModelContainer {
